package main //包
// node_mail go email模块（不是内置的，需要安装引用）
import (
	"log"
	"net/smtp"
...
func main() { // 入口函数
	e := email.NewEmail() //":"表示 定义并且附值
	e.From = "hunagmei <1593702111@qq.com>"
	// // [] ? Array? 多个用户发邮件
	// { }作为集合的容器，可添加多位用户
	// // go 静态类型   js 弱类型被认为不安全
	e.To = []string{"2189292157@qq.com"}
	e.Subject = "你妈喊你回家吃饭" //标题
	// // byte是 go类型 byte  字节流
	...
	// 25为端口号
	err := e.Send("smtp.qq.com:25", smtp.PlainAuth("", "1593702111@qq.com", "clrolusskueihdbe", "smtp.qq.com"))
	if err != nil {
		// console.log()
		// console.error()
		//打印错误 Fatal
